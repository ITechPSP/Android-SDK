package com.itechpsp.test_integration_kotlin
...
//              ,theme
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == PAY_ACTIVITY_REQUEST) {
            when (resultCode) {
                ITECHPSPActivity.RESULT_SUCCESS -> {}
                ITECHPSPActivity.RESULT_CANCELLED -> {}
                ITECHPSPActivity.RESULT_DECLINE -> {}
                ITECHPSPActivity.RESULT_FAILED -> {}
            }
            val error = data?.getStringExtra(ITECHPSPActivity.DATA_INTENT_EXTRA_ERROR)
            val token = data?.getStringExtra(ITECHPSPActivity.DATA_INTENT_EXTRA_TOKEN)
        }
    }

    // Payment Info
    internal fun getPaymentInfoOnlyRequiredParams(): ITECHPSPPaymentInfo {
        return ITECHPSPPaymentInfo(
    ...
    internal fun getPaymentInfoAllParams(): ITECHPSPPaymentInfo {
        return ITECHPSPPaymentInfo(
            PROJECT_ID, // project ID that is assigned to you
            "internal_payment_id_1", // payment ID to identify payment in your system
            1999, // 19.99
    ...
    // Additional
    internal fun setDMSPayment(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.setAction(ITECHPSPPaymentInfo.ActionType.Auth)
    }

    internal fun setActionTokenize(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.setAction(ITECHPSPPaymentInfo.ActionType.Tokenize)
    }

    internal fun setActionVerify(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.setAction(ITECHPSPPaymentInfo.ActionType.Verify)
    }

    internal fun setToken(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.token = "token"
    }

    internal fun setReceiptData(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.receiptData = "receipt data"
    }

    // if you want to hide the saved cards, pass the value - true
    internal fun setHideSavedWallets(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.hideSavedWallets = false
    }

    // For forced opening of the payment method, pass its code. Example: qiwi, card ...
    internal fun setForcePaymentMethod(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.forcePaymentMethod = "card"
    }

    internal fun setRecurrent(paymentInfo: ITECHPSPPaymentInfo) {
        val recurrentInfo = ITECHPSPRecurrentInfo(
            "R", // type
            "20", // expiry day
            "11", // expiry month
            "2030", // expiry year
...
//        )
    ...
    internal fun setKnownAdditionalFields(paymentInfo: ITECHPSPPaymentInfo) {
        paymentInfo.itechpspAdditionalFields = arrayOf(
            ITECHPSPAdditionalField(ITECHPSPAdditionalFieldEnums.AdditionalFieldType.customer_first_name, "Mark"),
            ITECHPSPAdditionalField(ITECHPSPAdditionalFieldEnums.AdditionalFieldType.billing_country, "US")
        )
    }

    // Setup 3D Secure parameters
    internal fun setupThreeDSecureParams(paymentInfo: ITECHPSPPaymentInfo) {
        val threeDSecureInfo = ITECHPSPThreeDSecureInfo()

        val threeDSecurePaymentInfo = ITECHPSPThreeDSecurePaymentInfo()
        ...
        val threeDSecureGiftCardInfo = ITECHPSPThreeDSecureGiftCardInfo()
        ...
        val threeDSecureCustomerInfo = ITECHPSPThreeDSecureCustomerInfo()
        ...
        val threeDSecureAccountInfo = ITECHPSPThreeDSecureAccountInfo() // object with account information on record with merchant
        ...
        val threeDSecureShippingInfo = ITECHPSPThreeDSecureShippingInfo() // object that contains shipment details
        ...
        val threeDSecureMpiResultInfo = ITECHPSPThreeDSecureMpiResultInfo() // object that contains information about previous customer authentication
        ...
        paymentInfo.itechpspThreeDSecureInfo = threeDSecureInfo
    }
}
